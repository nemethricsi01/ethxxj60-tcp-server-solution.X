# 1 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c" 2
# 41 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c"
...
# 42 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c" 2
...
# 44 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c" 2
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\stdbool.h" 1 3
# 45 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c" 2
# 1 "mcc_generated_files/TCPIPLibrary/physical_layer_interface.h" 1
# 46 "mcc_generated_files/TCPIPLibrary/physical_layer_interface.h"
...
# 47 "mcc_generated_files/TCPIPLibrary/physical_layer_interface.h" 2
...
# 46 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c" 2
...
# 95 "mcc_generated_files/TCPIPLibrary/../pin_manager.h"
void PIN_MANAGER_Initialize (void);
# 107 "mcc_generated_files/TCPIPLibrary/../pin_manager.h"
...
uint16_t errataTemp __attribute__((address(0xE7E)));
...
__attribute__((inline)) uint8_t ETH_EdataRead()
{
    __asm("movff EDATA,_errataTemp");
    return (uint8_t) errataTemp;
}

__attribute__((inline)) void ETH_EdataWrite(uint8_t d)
{
    __asm("movff WREG,EDATA");
}
...
# 219 "mcc_generated_files/TCPIPLibrary/ETHxxJ6x_driver.c"
    ...
    PHY_Write(PHCON1, phycon1_value);
    PHY_Write(PHCON2, 0x0110);
    PHY_Write(PHLCON, 0b01110101001010);
...
_Bool ETH_CheckLinkUp()
{
    uint32_t value;

    value = (uint32_t)PHY_Read(PHSTAT2);
    if(value & 0x0400)
    {
        ethData.up = 1;
        return 1;
...
void ETH_EventHandler(void)
{
   ...
   if (EIRbits.LINKIF)
    ...
    if(EIRbits.TXIF)
    {
        ...
        if( ethListSize > 0 )
        {
            if( EWRPT > ((((8192) - (((1518) + (7)) << 1))) + (((((1518) + (7)) << 1)) >> 1) ) )
            {
...
__attribute__((inline)) phyError_t PHY_WaitForBusy(void)
{
    phyError_t ret = NOERROR;
    uint8_t timeout;

    for(timeout = 0; timeout < 10;timeout++) __nop();
    timeout = 90;
    while(MISTATbits.BUSY && --timeout) __nop();
...
void ETH_Insert(char *data, uint16_t len, uint16_t offset)
{
    ...
    EWRPT = pHead->packetStart + offset + 1;
    while(len--)
    {
        ETH_EdataWrite(*data++);
    }
...
error_msg ETH_Copy(uint16_t len)
{
    ...
    timer = 2 * len;
    while(ECON1bits.DMAST!=0 && --timer) __nop();
    if(ECON1bits.DMAST==0)
    {
        ...
        if (tmp_len > ((((8192) - (((1518) + (7)) << 1)) - 1)) )
        {
            tmp_len = tmp_len - ((((8192) - (((1518) + (7)) << 1)) - 1));
            EDMAND = (0) + tmp_len;
...
error_msg ETH_Shift_Tx_Packets(void)
{
    ...
    if(ECON1bits.DMAST==0)
    {
        EDMADST = ((8192) - (((1518) + (7)) << 1));

        EDMAST = pTail->packetStart;
...
uint16_t ETH_TxComputeChecksum(uint16_t position, uint16_t len, uint16_t seed)
{
    ...
    ERDPT = pHead->packetStart + position + 1;
...
txPacket_t* ETH_NewPacket(void)
{
    ...
    while( index < (20) )
    {
        if( (_Bool)(txData[index].flags & (0x0001 << 2)) == 0 )
        {
            ...
            {
                txData[index].packetStart = ((8192) - (((1518) + (7)) << 1));
